...
const RegistrarVenta: React.FC<RegistrarVentaProps> = ({ onBack }) => {
  const { user } = useAuth();
...
const agregarAlCarrito = (producto: Producto) => {
  ...
  if (existente) {
    ...
    setCarrito((prev) =>
      prev.map((p) =>
        String(p.id) === String(producto.id_producto) // 👈 Comparación como string
    ...
    setCarrito((prev) => [
      ...prev,
      {
        id: String(producto.id_producto), // 👈 Asegúrate de guardar como string
  ...
  const cambiarCantidad = (id: string, nuevaCantidadRaw: number) => {
    const nuevaCantidad = Number.isNaN(nuevaCantidadRaw) ? 1 : Math.max(1, Math.floor(nuevaCantidadRaw));
    setCarrito((prev) =>
      prev.map((p) => {
        if (p.id !== id) return p;
...
const registrarVenta = async () => {
    ...
    for (const item of carrito) {
      const productoEnFuente = productos.find((p) => String(p.id_producto) === String(item.id));
      if (!productoEnFuente) {
        alert(`El producto ${item.nombre} ya no existe.`);
        return;
      }
      if (item.cantidadSeleccionada > productoEnFuente.stock) {
        alert(`La cantidad de ${item.nombre} supera el stock actual (${productoEnFuente.stock}). Ajusta la cantidad.`);
        return;
      }
    }

    const precioTotal = carrito.reduce((acc, p) => acc + p.precio * p.cantidadSeleccionada, 0);
    ...
    try {
      // 👇 Usa la instancia de axios configurada (incluye el token automáticamente)
      ...
      // Recargar productos
      const resProductos = await api.get("/producto");
      setProductos(Array.isArray(resProductos.data) ? resProductos.data : []);
      
    } catch (error: any) {
      console.error("❌ Error al registrar venta:", error);
      alert(`Ocurrió un error al registrar la venta: ${error.response?.data?.message || error.message || 'Error desconocido'}`);
    }
  };
      ...
      <div className="cliente-section">
            ...
            {clientesFiltrados.length === 0 ? (
              ...
              clientesFiltrados.slice(0, 8).map((cliente, index) => (
                <div
                  ...
                  className={`cliente-card ${clienteSeleccionado?.id === cliente.id ? "seleccionado" : ""}`}
                  onClick={() => setClienteSeleccionado(cliente)}
                >
                  <p>
                    <strong>{cliente.nombre}</strong> — DNI: {cliente.dni ?? "N/A"}
                  </p>
        ...
        {clienteSeleccionado && (
          <div className="cliente-seleccionado">
            Cliente seleccionado: <strong>{clienteSeleccionado.nombre} - {clienteSeleccionado.dni}</strong>
          </div>
        )}
      </div>
              ...
              return (
                <div key={producto.id_producto} className="producto-card">
                  <div className="producto-img-wrap">
                    {producto.imagen ? (
                      <img src={producto.imagen} alt={producto.nombre} onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = "none"; }} />
                    ) : null}
                  </div>
                  <h4>{producto.nombre}</h4>
                  {producto.descripcion && <p className="prod-desc">{producto.descripcion}</p>}
                  <p className="stock-line">Stock disponible: <span className="stock-badge">{producto.stock}</span></p>
                  <p><strong>${producto.precio}</strong></p>
                  <button
                    onClick={() => agregarAlCarrito(producto)}
                    disabled={!puedeAgregar}
                    className={`agregar-btn ${!puedeAgregar ? "disabled" : ""}`}
                    title={!puedeAgregar ? (agotado ? "Sin stock" : "Ya agregaste la cantidad máxima") : "Agregar al carrito"}
                  >
                    Agregar
                  </button>
                </div>
              ...
              {carrito.map((p) => (
                <tr key={p.id}>
                  ...
                  <td>
                    ...
                    />
                    <div className="stock-small"> / {p.cantidadStock} disponibles</div>
                  </td>
                  <td>${p.precio}</td>
                  <td>${(p.precio * p.cantidadSeleccionada).toFixed(2)}</td>
                  <td>
